...
export default class ClientesController {
   ...
   * /api/clientes/{id}:
   *   put:
   *     summary: Substitui cliente completamente (todos os campos obrigatórios)
   *     tags: [Clientes]
   *     security:
   *       - bearerAuth: []
   *     parameters:
  ...
  replace = async (req: Request, res: Response): Promise<void> => {
    ...
    const { id } = req.params;
    const result = await this.service.replaceCliente(id, req.body);
    ...
    res.status(200).json(
      createSuccessResponse(result.data, 'Cliente substituído com sucesso')
    );
  };
